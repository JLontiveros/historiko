...
const Tirad3d = () => {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('tirad3dToastShown') === null) {
      localStorage.setItem('tirad3dToastShown', 'false');
    }
  
    const fetchVideo = async () => {
      try {
        const videoRef = ref(storage, 'tiradvid.mp4');
        const url = await getDownloadURL(videoRef);
        setVideoUrl(url);
  
        // After fetching the video, update localStorage and show the toast if not previously shown
        if (localStorage.getItem('tirad3dToastShown') === 'false') {
          const userName = user ? user.name || user.username : 'Kaibigan';
          toast.info(
            `Pagbati, ${userName}! Magpatuloy at panoorin ang 3d 'Labanan sa Tirad Pass'`, 
            {
              position: "top-right",
              autoClose: 5000,
          ...
          localStorage.setItem('tirad3dToastShown', 'true');
        }
      } catch (error) {
